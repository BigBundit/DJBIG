...
export const Lane: React.FC<LaneProps> = memo(({ config, active, onTrigger, onRelease, theme, isOverdrive, graphicsQuality }) => {
    ...
    const renderReceptor = () => {
        if (theme.id === 'ignore') {
            ...
            if (isOverdrive) {
                 appearanceClasses = isLow ? `bg-amber-500 border-white/40 ${active ? 'translate-y-1' : ''}` : `bg-amber-400 border-white/40 ${active ? 'translate-y-1 brightness-125' : 'opacity-100'}`;
            } else {
            ...
            return (
                <div className={`${buttonBase} ${appearanceClasses}`}>
                    {!isOverdrive && <div className={`w-full h-full absolute inset-0 rounded-lg ${active ? (isLow ? `bg-${config.color.base}-500/50` : `opacity-30 bg-${config.color.base}-500`) : ''}`}></div>}
                    {!isLow && <div className="absolute top-0 left-0 right-0 h-1/2 bg-white/5 rounded-t-lg"></div>}
                    {isOverdrive && !isLow && <div className="absolute inset-0 bg-white/10 pointer-events-none"></div>}
                </div>
            );
            ...
            const glowColor = isOverdrive ? 'bg-amber-300' : (active ? `bg-${config.color.base}-500 ${!isLow ? 'shadow-[0_0_15px_currentColor]' : ''}` : 'bg-slate-800');
            return (
             ...
             const shadowClass = isLow ? '' : (isOverdrive ? 'shadow-[0_0_15px_rgba(251,191,36,0.5)]' : (active ? 'shadow-[0_0_30px_rgba(236,72,153,1)]' : 'shadow-[0_0_15px_rgba(236,72,153,0.3)]'));
            return (
                <div className={`absolute bottom-3 left-1 right-1 h-16 flex items-end justify-center transition-all duration-75 z-30`}>
                    <div className={`relative w-full rounded-md border-b-4 transition-all duration-75 flex items-center justify-center overflow-hidden ${active ? 'h-14 translate-y-1' : 'h-14'} ${bgClass} ${shadowClass}`}>
                        ...
                        {!isLow && <div className={`absolute inset-2 rounded blur-sm transition-opacity duration-75 ${active || isOverdrive ? 'bg-white opacity-40' : 'bg-pink-500 opacity-10'}`}></div>}
                        {!isLow && <div className="absolute top-0 inset-x-0 h-1/2 bg-white/5 rounded-t-md"></div>}
                    </div>
                </div>
            );
        } else {
            const lineClass = isOverdrive ? (isLow ? 'bg-amber-500' : 'bg-amber-400 shadow-[0_0_10px_white]') : (active ? (isLow ? `bg-${config.color.base}-400` : `bg-${config.color.base}-400 shadow-[0_0_20px_rgba(255,255,255,0.8)]`) : `bg-slate-600/50 border-x border-${config.color.base}-500/30`);
            return (
    ...
    return (
        <div className={`relative flex-1 h-full border-r border-white/5 last:border-r-0 transition-colors duration-50 ${bgGradient} touch-none select-none`} onPointerDown={(e) => { e.preventDefault(); onTrigger(); }} onPointerUp={(e) => { e.preventDefault(); onRelease(); }} onPointerLeave={(e) => { e.preventDefault(); onRelease(); }}>
            ...
            {renderReceptor()}
            {active && !isLow && <div className={`absolute bottom-0 left-0 right-0 top-0 bg-gradient-to-t ${hitGradient} to-transparent opacity-40 pointer-events-none z-10`}></div>}
        </div>
    );
});
